...
def Buku_list(request):
    ...
    return render(request, 'list_buku/Admin_list.html', {'books': books})

def Peminjam_buku_list(request):
    books = Buku.objects.all()
    return render(request, 'list_buku/Peminjam_list.html', {'books': books})
...
def create_Buku(request):
    if request.method == 'POST':
        ...
        form = BukuForm()
    return render(request, 'form_buku/Admin_form.html', {'form': form})
